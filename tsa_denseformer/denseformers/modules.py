...
class DWAModules(torch.nn.Module):
    ...
    def __init__(self, n_blocks, dilation=1, period=1):
        ...
        self.period = period
        self.alphas = torch.nn.ModuleList([torch.nn.Linear(
            (i+1+dilation)//dilation, 1, bias=False) if (i+1) % period == 0 else None for i in range(n_blocks)])
    ...
    def init_accumulators(self, x):
        ...
        for i in range(self.dilation):
            ...
            x_accs.append((torch.zeros((current_group_size, *x.shape),
    ...
    def forward(self, x, block_idx):
        ...
        if (block_idx+1) % self.period == 0:
            x = torch.tensordot(self.alphas[block_idx].weight.view(
                -1), self.accumulators[(block_idx+1) % self.dilation][1], dims=1)
        return x
